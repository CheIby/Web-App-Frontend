...
export const AddOrderBox = (props) => {
    ...
    const submitForm=async()=>{
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
          axios.post(`${process.env.REACT_APP_API}/Order/AddOrder`,{restaurant:selectedValue,detail:detail,receiveLocation:receiveLocation,ifDoneScore:sum},{
            headers:{
  ...
  return (
          <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm z-50 outline-none focus:outline-none">
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex px-5 items-center mt-3">
                    <p className="block w-[30%] text-sm font-medium text-black mr-3">Select Restaurant</p>
                    <select value={selectedValue} onChange={handleSelectChange} className="bg-black border w-[70%] text-white text-sm rounded-lg  block p-2.5  ">
            ...
            <button onClick={handleClick} className="w-full p-2 bg-black rounded-lg text-white">Add</button>
            </div>
            <div className="w-full  my-3 px-5">
                <p>Recieve Location</p>
                <input className="border-b-2 indent-2 w-full" type="text" onChange={e=>setReceiveLocation(e.target.value)}/>
            </div>
                {/*footer*/}
